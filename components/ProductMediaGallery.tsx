...
export default function ProductMediaGallery({ title, media, colorVariants = [], selectedColor = "", onColorChange }: { title: string; media: ProductMedia[]; colorVariants?: Array<{ name: string; media?: ProductMedia[] }>; selectedColor?: string; onColorChange?: (color: string) => void }) {
  ...
  const safeActiveIndex = Math.min(activeIndex, Math.max(selectedGalleryMedia.length - 1, 0));
  const activeMedia = selectedGalleryMedia[safeActiveIndex];
  ...
  const move = (direction: 1 | -1) => {
    ...
    setActiveIndex((current) => (Math.min(current, selectedGalleryMedia.length - 1) + direction + selectedGalleryMedia.length) % selectedGalleryMedia.length);
  };
  ...
  return (
    <div data-liquid-surface className="liquid-panel relative rounded-2xl bg-black/10 p-2">
      ...
      >
        <div key={`${activeMedia.url}-${safeActiveIndex}`} className="h-full w-full animate-[hadx-media-in_320ms_ease-out] [transform-style:preserve-3d]">
          {activeMedia.type === "video" ? (
            ...
            <img src={activeMedia.url} alt={`${title}, image ${safeActiveIndex + 1} of ${selectedGalleryMedia.length}`} loading={safeActiveIndex === 0 ? "eager" : "lazy"} decoding="async" className={`h-full w-full object-contain ${activeMedia.type === "image" ? "cursor-zoom-in" : ""}`} onClick={activeMedia.type === "image" ? () => setIsViewerOpen(true) : undefined} />
          )}
        </div>
        ...
        {selectedGalleryMedia.length > 1 ? (
          <>
            ...
            <span className="pointer-events-none absolute left-4 top-4 rounded-full border border-white/15 bg-black/15 px-3 py-1 text-[8px] font-mono uppercase tracking-[0.18em] text-white/60">Swipe / drag {safeActiveIndex + 1} / {selectedGalleryMedia.length}</span>
          </>
        ) : null}
      </div>

      {activeMedia.type === "image" ? <ProductImageLightbox src={activeMedia.url} alt={`${title}, image ${safeActiveIndex + 1}`} open={isViewerOpen} onClose={() => setIsViewerOpen(false)} /> : null}
