  ...
  return (
    <div className="flex flex-col items-start gap-1">
      <button
        ...
        aria-pressed={active}
        disabled={loading}
        className={`liquid-ui group relative inline-flex items-center gap-2 rounded-lg border px-6 py-3 transition-transform duration-150 ${active ? "bg-hadx-gold/10 border-hadx-border-glow shadow-gold-glow" : "border-hadx-border hover:border-hadx-border-glow"} ${pulsing ? "scale-[1.03]" : "scale-100"} ${loading ? "cursor-wait opacity-70" : ""}`}
      >
        ...
        <span className="text-[11px] font-mono uppercase tracking-[0.2em] text-hadx-gold-light">{active ? "IN VAULT" : "STASH IN VAULT"}</span>
      </button>
      {message ? <span role="status" className="px-1 text-[9px] font-mono uppercase tracking-[0.14em] text-white/45">{message}</span> : null}
    </div>
  );
}
